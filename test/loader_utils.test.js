...
describe.each([
  ...
  it("Assigns loaders to the `.loaders` property", () => {
    const loaders = {
      a: makeLoader(Promise.resolve(9)),
      b: makeLoader(Promise.resolve("abc")),
    };
    const lm = fn(loaders);
  ...
  describe(".loadResources()", () => {
    ...
    it("Resolves (with the correct value) when the single promise is resolved", async () => {
      var /** @type {(value: any) => void} */ resolvePromise,
        promiseDone = false;
    ...
    it("Resolves (with the correct value) only after all promises resolved", async () => {
      ...
      const promises = {
        a: new Promise((resolve) => {
          resolvers.a.resolve = resolve;
        }),
        b: new Promise((resolve) => {
          resolvers.b.resolve = resolve;
        }),
      };
      ...
      await wait(40);
      expect(promiseDone).toBe(true);
      expect(resolveValue).toStrictEqual({
        a: resolvers.a.value,
        b: resolvers.b.value,
      });
...
describe("makeLoaderMerge.<return>", () => {
  it("Assigns loaders to returned object", () => {
    const loaders = {
      a: makeLoader(Promise.resolve(13.169)),
      b: makeLoader(Promise.resolve("8910")),
    };
  ...
  it("Doesn't assign blocked properties", () => {
    {
      const loaders = {
        a: makeLoader(Promise.resolve(13.169)),
        promises: makeLoader(Promise.resolve("8910")),
      };
      const lm = makeLoaderMerge(loaders);
      expect(lm.a).toBe(loaders.a);
    ...
    }
    {
      const loaders = {
        a: makeLoader(Promise.resolve(13.169)),
        toString: makeLoader(Promise.resolve("8910")),
      };
    ...
    }
    {
      const loaders = {
        a: makeLoader(Promise.resolve(13.169)),
        constructor: makeLoader(Promise.resolve("8910")),
      };
      const lm = makeLoaderMerge(loaders);
      expect(lm.a).toBe(loaders.a);
  ...
  it("Doesn't modify the argument", async () => {
    ...
    const loadersOrig = {
      promises: makeLoader(promiseA),
      toString: makeLoader(promiseB),
    };
