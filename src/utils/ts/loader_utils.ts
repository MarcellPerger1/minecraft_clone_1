...
export class LoaderMerge<L extends { [k: PropertyKey]: LoaderT }> {
  ...
  startPromises(): this {
    ...
    this.promises = fromEntries(
      getEntries(this.loaders).map(([k, v]) => [
        k as keyof L,
        v.loadResources() as any,
      ])
    );
    return this;
  }
  ...
  async loadResources(): Promise<LoadersToValuesT<L>> {
    // aargh typescript!!
    this.startPromises();
    const promiseList = getEntries(
      this.promises! as { [k: PropertyKey]: unknown }
    ).map(async ([k, v]) => [k, await v]) as Promise<[PropertyKey, unknown]>[];
...
export function makeLoaderMerge<L extends { [k: PropertyKey]: LoaderT }>(
  loaders: L
): LoaderMerge<L> & L {
...
function fromEntries<KT extends PropertyKey, VT>(
  entries: [KT, VT][]
): { [k in KT]: VT } {
