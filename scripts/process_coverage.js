  ...
  while (curr.line <= lines.length) {
    ...
    if (char == "(") parenCount += 1;
      ...
      if (parenCount == 0) {
        ...
        return {
          line: curr.line,
          column: rest.match(/[^{]*\{/ds).indices[0][1] + curr.column,
        };
      }
...
async function pptToIst(pptCov) {
  ...
  assert(
    rawCov,
    "rawScriptCoverage not found out puppeteer coverage json (ensure it is using includeRawScriptCoverage)"
  );
  ...
  if (DEBUG) {
    ...
    await fs.promises.writeFile(
      `./_temp/${_pathToFilename(rawCov.url)}-v8.json`,
      JSON.stringify(rawCov)
    );
    await fs.promises.writeFile(
      `./_temp/${_pathToFilename(rawCov.url)}-ist.json`,
      JSON.stringify(ist)
    );
    await fs.promises.writeFile(
      `./_temp/${_pathToFilename(rawCov.url)}-n_ist.json`,
      JSON.stringify(normalIst)
    );
  }
  return normalIst;
}

const mapJest = libCoverage.createCoverageMap(
  JSON.parse(
...
const v8CovPpt = JSON.parse(
  await fs.promises.readFile("./test/coverage-ppt-raw/out.json", "utf8")
);
assert(Array.isArray(v8CovPpt));
const istCovPpt = (await Promise.all(v8CovPpt.map(pptToIst))).filter(
  (v) => v != null
);
...
istanbulReports.create("lcov").execute(
  libReport.createContext({
    ...
    coverageMap: libCoverage.createCoverageMap(
      JSON.parse(
        await fs.promises.readFile("./test/coverage-puppeteer/out.json", "utf8")
      )
    ),
  })
);
